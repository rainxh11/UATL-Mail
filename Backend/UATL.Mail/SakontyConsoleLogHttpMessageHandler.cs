...
namespace UATL.Mail
{
    ...
    {
        public static Colorful.Formatter FormatStatusCode(int code)
        {
            switch (code)
            {
                case var number when (number >= 100 && number < 200):
                    return new Colorful.Formatter($"{code}", Color.Cyan);

                case var number when (number >= 200 && number < 300):
                    return new Colorful.Formatter($"{code}", Color.LimeGreen);

                case var number when (number >= 300 && number < 400):
                    return new Colorful.Formatter($"{code}", Color.Purple);

                case var number when (number >= 400 && number < 500):
                    return new Colorful.Formatter($"{code}", Color.Orange);

                default:
                case var number when (number >= 500 && number < 600):
                    return new Colorful.Formatter($"{code}", Color.Red);
            }
        }
