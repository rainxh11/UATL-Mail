...
const cacheName = 'sw-precache-v3-sw-precache-' + (self.registration ? self.registration.scope : '')
...
const createCacheKey = function(originalUrl, paramName, paramValue,
...
const isPathWhitelisted = function(whitelist, absoluteUrlString) {
 ...
 return whitelist.some((whitelistedPathRegex) => {
 return path.match(whitelistedPathRegex)
 })
 }

const stripIgnoredUrlParameters = function(originalUrl,
...
self.addEventListener('install', (event) => {
 ...
 return cleanResponse(response).then((responseToCache) => {
